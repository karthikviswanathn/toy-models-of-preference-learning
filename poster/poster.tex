...
\begin{posterbox}
\posterbody
Alignment via post-training learns a shallow safety ``wrapper'' over pretrained capabilities (Jain et al., 2024). Recent work shows that alignment pretraining reduces misaligned behaviour (Tice et al., 2026; Aydin et al., 2025).
...
\vspace{4pt}
\posterbody
The \textbf{PC$^2$-augmented probe} appends squared principal components to the residual-stream activations $[\mathbf{h};\, \mathrm{PC}_1^2, \dots, \mathrm{PC}_k^2]$, letting it detect features encoded in the \textit{radius} of a circular manifold rather than a linear direction.

\vspace{4pt}
Across 126 configs, \POST{} improves substantially with PC$^2$-augmentation, especially at post-MLP, consistent with parity being encoded \textbf{nonlinearly} in the Fourier ring geometry.
\PTG{} also benefits at attention heads, confirming nonlinear parity encoding early in the network.
\end{posterbox}
...
\begin{tcolorbox}[
  ...
  boxrule=0pt,
  arc=0pt,
  left=0pt, right=0pt, top=0.3cm, bottom=0.3cm,
  boxsep=0pt,
]
\phantom{x}
\end{tcolorbox}
